...
local function on_SystemEvent(e)
	...
	if _type == "applicationStart" then -- 앱이 시작될 때
		...
		local tm = TabMenu.create({"Icon.png", "Icon.png", "Icon.png", "Icon.png"})
		tm.y = __appContentHeight__ - tm.height
		
		tm.select(1) -- 첫 번째 메뉴 선택
		
		-- 메뉴 선택 이벤트 핸들러
		local function on_Selected(e)
			local options = {
				effect = ((e.oldIndex == nil or (e.oldIndex < e.newIndex)) and "slideLeft" or "slideRight"),
				time = 300,
				params = nil
			}
			composer.gotoScene("scenes.Scene"..e.newIndex, options)
		end
		tm:addEventListener("selected", on_Selected)
		--==========================
